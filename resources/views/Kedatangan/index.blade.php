    ...
    <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{$no}}</td>
                    <td>{{$kedatangan->jam}}</td>
                    <td>{{ date("d F Y", strtotime($kedatangan->tanggal)) }}</td>
                    <td>{{$kedatangan->nama_po}}</td>
                    <td>{{$kedatangan->plat_nomor}}</td>
                    <td>PEKANBARU</td>
                    <td>{{$kedatangan->tujuan}}</td>
                    <td>{{$kedatangan->penumpang}}</td>
                    <td>{{$kedatangan->nama_kategori}}</td>
                    @role('Admin|Pelaksana')
                    <td>

                        <div class="row justify-content-center">
                            <a href="{{route('kedatangan.edit', $kedatangan->id)}}" class="btn btn-info mt-1 mb-1 mr-1 ml-1"><i
                                    class="fas fa-edit"></i></a>

                            <a href="{{ route('kedatangan.delete', $kedatangan->id) }}" class="btn btn-danger mt-1 mb-1 mr-1 ml-1"
